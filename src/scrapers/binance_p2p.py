...
def get_binance_p2p_csv(ticker= 'USDT', buyOrSell= 'BUY', fiat= 'ARS'): #U: p2p asking prices from Binance, returns DataFrame
  ...
  os.makedirs(cfg.get_data_dir()+'/binance_p2p', exist_ok= True) #A: Create dir if it does not exist 
  fpath= cfg.get_data_dir()+'/binance_p2p/'+ticker+'_'+fiat+'_'+buyOrSell+'_'+datetime.now().strftime('%Y-%m-%d-%H')+'.csv' #A: Save at most one per hour
